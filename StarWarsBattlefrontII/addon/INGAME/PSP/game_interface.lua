-- 
-- game_interface.lua (PSP version 'alt addon system') 
--
...
----------------------------------------------------------
-- if you want more stuff done at the end of 'game_interface', override this function in 
-- 'ingame_overrides.lua' 
function GameInterfaceEnd()
	-- empty function, but give modder a chance to ove do stuff at the end of the file 
end 

--- Give modder a chance to override scripts used 'ingame'
if( ScriptCB_IsFileExist("addon\\000\\ingame_overrides.lvl") == 1) then 
	ReadDataFile("addon\\000\\ingame_overrides.lvl")
	ScriptCB_DoFile("ingame_overrides")
end 
----------------------------------------------------------
...
if(gPlatformStr ~= "PS2" and gPlatformStr ~= "PSP" ) then
...
if gPlatformStr == "PSP" then 
	ScriptCB_DoFile("ifs_mcstats")
	ScriptCB_DoFile("ifs_mpmcstats")
	ScriptCB_DoFile("ifs_mcfinalstats")
end 
...
-- Call this function so that modder may have overridden to potentially do more stuff
GameInterfaceEnd()
